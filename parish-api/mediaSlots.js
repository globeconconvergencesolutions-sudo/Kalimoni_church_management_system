...
const homeParishLife = parishLifeDefaults.map((p, i) =>
  slot({
...
const ministrySlots = ministryDefaults.map((m, i) =>
  slot({
    key: `ministries.${m.id}`,
    page: 'ministries',
...
export function getSlotDef(key) {
  return MEDIA_SLOT_DEFS.find(s => s.key === key)
}

export function slotsForPage(pageId) {
  return MEDIA_SLOT_DEFS.filter(s => s.page === pageId).sort((a, b) => a.sortOrder - b.sortOrder)
}

export function cloudinaryPublicId(root, relativePath) {
